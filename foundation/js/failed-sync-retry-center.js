...
function normalizeJob(source, row, fallbackType) {
  ...
  const retryCount = Number(pick(row, ["retry_count", "attempts"], 0)) || 0;
...
async function loadTableJobs() {
  ...
  const loadMoodle = async () => {
    const { data, error } = await supabase
      .from("moodle_sync")
      .select("*")
      .or("status.ilike.%fail%,status.ilike.%error%,error_message.not.is.null")
      .order("updated_at", { ascending: false })
      .limit(500);
    if (error) {
      if (isMissingTable(error)) {
        missing.push("moodle_sync");
        return [];
      }
      return [];
    }
    return (data || []).map((r) => normalizeJob("moodle_sync", r, "Moodle"));
  };
