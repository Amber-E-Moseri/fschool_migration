...
begin
  ...
  begin
    ...
    -- Use teacher's specific fellowship code, fall back to REGIONAL
    v_fellowship_codes := case
      when v_teacher.fellowship_code is not null
        then array[v_teacher.fellowship_code]
      else array['REGIONAL']::text[]
    end;

    insert into public.class_options (
      class_option_id, class_id, teacher_id, teacher_name,
      fellowship_codes, group_id, subgroup_id, day, class_time,
      active, enrollment_open, deleted_at, updated_by
    )
    values (
      v_co_id, v_co_id, v_avail.teacher_id,
      coalesce(v_teacher.full_name, ''),
      v_fellowship_codes,
      coalesce(v_teacher.group_id, ''),
      coalesce(v_teacher.subgroup_id, ''),
      v_day, v_time, true, true, null, p_actor_id
    )
    ...
    insert into public.class_slots (
      class_slot_id, class_option_id, teacher_id, teacher_name,
      group_id, subgroup_id, batch_id, status, current_enrolment, updated_by
    )
    values (
      v_cs_id, v_co_id, v_avail.teacher_id,
      coalesce(v_teacher.full_name, ''),
      coalesce(v_teacher.group_id, ''),
      coalesce(v_teacher.subgroup_id, ''),
      v_batch_id, 'Active', 0, p_actor_id
    )
    ...
    update public.teacher_availability set
      status                    = 'Available',
