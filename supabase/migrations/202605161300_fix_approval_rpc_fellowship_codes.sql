-- Fix: approve_teacher_availability_atomic was inserting fellowship_codes = '{}'
-- causing class_options to be invisible on the registration form.
-- Now populates from fellowship_map by subgroup_id, falling back to REGIONAL.
-- Keeps current authorization behavior (admin OR superadmin).
...
begin
  ...
  begin
    ...
    -- Resolve fellowship codes from fellowship_map by subgroup_id.
    -- Falls back to REGIONAL so the class is always visible somewhere.
    select coalesce(
      array_agg(distinct fm.fellowship_code),
      array['REGIONAL']::text[]
    )
    into v_fellowship_codes
    from public.fellowship_map fm
    where fm.active = true
      and fm.subgroup_id = v_teacher.subgroup_id;

    if v_fellowship_codes is null or array_length(v_fellowship_codes, 1) = 0 then
      v_fellowship_codes := array['REGIONAL']::text[];
    end if;
    ...
    -- Backfill existing class_options with empty fellowship_codes for this subgroup.
    update public.class_options co
    set fellowship_codes = v_fellowship_codes,
        updated_at = now()
    where co.subgroup_id = v_teacher.subgroup_id
      and (co.fellowship_codes = '{}'::text[] or co.fellowship_codes is null);
